...
// delegate method
protocol APChartViewDelegate1 {
    func didSelectNearDataPoint1(_ selectedDots:[String:APChartPoint1])
}


@IBDesignable class APChartView1:UIView,UIScrollViewDelegate{
    var collectionLines:[APChartLine1] = []
    var collectionMarkers:[APChartMarkerLine1] = []
    var delegate: APChartViewDelegate1!
    ...
    var offsetX: Offset1 = Offset1(min:0.0, max:1.0)
    var offsetY: Offset1 = Offset1(min:0.0, max:1.0)
    ...
    var lineMax :APChartMarkerLine1?
    var lineMin :APChartMarkerLine1?
    var removeAll: Bool = false
    
    override init(frame: CGRect) {
        super.init(frame: frame)
        let line = APChartLine1(chartView: self, title: "mio", lineWidth: 2.0, lineColor: self.colors[1])
        ...
        _ = APChartMarkerLine1(chartView: self, title: "x marker", x: 85.0, lineColor: UIColor.red )
    ...
    func addLine(_ line:APChartLine1){
    ...
    func addMarkerLine(_ title:String, x:CGFloat) {
        ...
        let line = APChartMarkerLine1(chartView: self, title: title, x: x, lineColor: UIColor.red)
        collectionMarkers.append(line)
    }
    
    
    func addMarkerLine(_ title:String, y:CGFloat){
        let line = APChartMarkerLine1(chartView: self, title: title, y: y, lineColor: UIColor.blue)
        collectionMarkers.append(line)
    ...
    override func draw(_ rect: CGRect) {
        ...
        if showMax {
            lineMax = APChartMarkerLine1(chartView: self, title: "Max", y: offsetY.maxValue, lineColor: UIColor.blue)
        }
        
        if showMin {
            lineMin = APChartMarkerLine1(chartView: self, title: "Min", y: offsetY.minValue, lineColor: UIColor.blue)
        }
    ...
    func calculateOffsets()  {
        offsetX = Offset1(min:0.0, max:1.0)
        offsetY = Offset1(min:0.0, max:1.0)
        ...
        offsetX = Offset1(min:p.dot.x, max:p.dot.x )
        offsetY = Offset1(min:p.dot.y, max:p.dot.y )
        
        for line in collectionLines {
            
            for curr:APChartPoint1 in line.dots {
    ...
    func getClosetLineDot(_ selectedPoint:CGPoint) -> [String:APChartPoint1]?{
        var delta:CGFloat = 100000.0
        var diff:CGFloat = 0.0
        var selectedDot:[String:APChartPoint1] = [:]
        for line in collectionLines {
            delta = 5.0
    ...
    func handleTouchEvents(_ touches: Set<NSObject>, withEvent event: UIEvent) {
        ...
        if let closestDots = getClosetLineDot(selectedPoint) {
            delegate?.didSelectNearDataPoint1(closestDots)
        }
