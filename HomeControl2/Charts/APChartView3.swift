...
// delegate method
protocol APChartViewDelegate3 {
    func didSelectNearDataPoint3(_ selectedDots:[String:APChartPoint3])
}


@IBDesignable class APChartView3:UIView,UIScrollViewDelegate{
    var collectionLines:[APChartLine3] = []
    var collectionMarkers:[APChartMarkerLine3] = []
    var delegate: APChartViewDelegate3!
    ...
    var offsetX: Offset3 = Offset3(min:0.0, max:1.0)
    var offsetY: Offset3 = Offset3(min:0.0, max:1.0)
    ...
    var lineMax :APChartMarkerLine3?
    var lineMin :APChartMarkerLine3?
    var removeAll: Bool = false
    
    override init(frame: CGRect) {
        super.init(frame: frame)
        let line = APChartLine3(chartView: self, title: "mio", lineWidth: 2.0, lineColor: self.colors[1])
        ...
        _ = APChartMarkerLine3(chartView: self, title: "x marker", x: 85.0, lineColor: UIColor.red )
    ...
    func addLine(_ line:APChartLine3){
    ...
    func addMarkerLine(_ title:String, x:CGFloat) {
        ...
        let line = APChartMarkerLine3(chartView: self, title: title, x: x, lineColor: UIColor.red)
        collectionMarkers.append(line)
    }
    
    
    func addMarkerLine(_ title:String, y:CGFloat){
        let line = APChartMarkerLine3(chartView: self, title: title, y: y, lineColor: UIColor.blue)
        collectionMarkers.append(line)
    ...
    override func draw(_ rect: CGRect) {
        ...
        if showMax {
            lineMax = APChartMarkerLine3(chartView: self, title: "Max", y: offsetY.maxValue, lineColor: UIColor.blue)
        }
        
        if showMin {
            lineMin = APChartMarkerLine3(chartView: self, title: "Min", y: offsetY.minValue, lineColor: UIColor.blue)
        }
    ...
    func calculateOffsets()  {
        offsetX = Offset3(min:0.0, max:1.0)
        offsetY = Offset3(min:0.0, max:1.0)
        ...
        offsetX = Offset3(min:p.dot.x, max:p.dot.x )
        offsetY = Offset3(min:p.dot.y, max:p.dot.y )
        
        for line in collectionLines {
            
            for curr:APChartPoint3 in line.dots {
    ...
    func drawXLabels() {
        ...
        while step < gridLinesX {
            ...
            let label = UILabel(frame: CGRect(x: x-12.0, y: pointZero.y+12.0, width: pointZero.x-55, height: 16.0))
            label.font = UIFont.boldSystemFont(ofSize: 10)
            label.textAlignment = NSTextAlignment.left
            label.transform = CGAffineTransform(rotationAngle: CGFloat.pi * 7 / 18)
            var textX = offsetX.min + xValue_delta*step
            label.text = "\(czas):00" //czas
    ...
    func getClosetLineDot(_ selectedPoint:CGPoint) -> [String:APChartPoint3]?{
        var delta:CGFloat = 100000.0
        var diff:CGFloat = 0.0
        var selectedDot:[String:APChartPoint3] = [:]
        for line in collectionLines {
            delta = 5.0
    ...
    func handleTouchEvents(_ touches: Set<NSObject>, withEvent event: UIEvent) {
        ...
        if let closestDots = getClosetLineDot(selectedPoint) {
            delegate?.didSelectNearDataPoint3(closestDots)
        }
