...
// delegate method
protocol APChartViewDelegate2 {
    func didSelectNearDataPoint2(_ selectedDots:[String:APChartPoint2])
}


@IBDesignable class APChartView2:UIView,UIScrollViewDelegate{
    var collectionLines:[APChartLine2] = []
    var collectionMarkers:[APChartMarkerLine2] = []
    var delegate: APChartViewDelegate2!
    ...
    var offsetX: Offset2 = Offset2(min:0.0, max:1.0)
    var offsetY: Offset2 = Offset2(min:0.0, max:1.0)
    ...
    var lineMax :APChartMarkerLine2?
    var lineMin :APChartMarkerLine2?
    var removeAll: Bool = false
    
    override init(frame: CGRect) {
        super.init(frame: frame)
        let line = APChartLine2(chartView: self, title: "mio", lineWidth: 2.0, lineColor: self.colors[1])
        ...
        _ = APChartMarkerLine2(chartView: self, title: "x marker", x: 85.0, lineColor: UIColor.red )
    ...
    func addLine(_ line:APChartLine2){
    ...
    func addMarkerLine(_ title:String, x:CGFloat) {
        ...
        let line = APChartMarkerLine2(chartView: self, title: title, x: x, lineColor: UIColor.red)
        collectionMarkers.append(line)
    }
    
    
    func addMarkerLine(_ title:String, y:CGFloat){
        let line = APChartMarkerLine2(chartView: self, title: title, y: y, lineColor: UIColor.blue)
        collectionMarkers.append(line)
    ...
    override func draw(_ rect: CGRect) {
        ...
        if showMax {
            lineMax = APChartMarkerLine2(chartView: self, title: "Max", y: offsetY.maxValue, lineColor: UIColor.blue)
        }
        
        if showMin {
            lineMin = APChartMarkerLine2(chartView: self, title: "Min", y: offsetY.minValue, lineColor: UIColor.blue)
        }
    ...
    func calculateOffsets()  {
        offsetX = Offset2(min:0.0, max:1.0)
        offsetY = Offset2(min:0.0, max:1.0)
        ...
        offsetX = Offset2(min:p.dot.x, max:p.dot.x )
        offsetY = Offset2(min:p.dot.y, max:p.dot.y )
        
        for line in collectionLines {
            
            for curr:APChartPoint2 in line.dots {
    ...
    func getClosetLineDot(_ selectedPoint:CGPoint) -> [String:APChartPoint2]?{
        var delta:CGFloat = 100000.0
        var diff:CGFloat = 0.0
        var selectedDot:[String:APChartPoint2] = [:]
        for line in collectionLines {
            delta = 5.0
    ...
    func handleTouchEvents(_ touches: Set<NSObject>, withEvent event: UIEvent) {
        ...
        if let closestDots = getClosetLineDot(selectedPoint) {
            delegate?.didSelectNearDataPoint2(closestDots)
        }
